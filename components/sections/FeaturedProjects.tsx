                    ...
                    <div className="flex flex-wrap pt-red -mx-4" key={project.project?._id}>
                        { project.images?.[0] && (
                            ...
                            <Link 
                                ...
                                >
                                <ImageWithBlur image={project.images[0]} sizes="(max-width: 768px) 100vw , 50vw" optionalAlt="Img Project"/>
                            </Link>
                        </div>
                        )}
                        {project.images?.[1] && (
                            <div className="w-full md:w-6/12 hidden md:flex px-4 ">
                                <Link href={`./projects/${project.project?.slug.current}`} className='hidden md:block'>
                                    <ImageWithBlur image={project.images[1]} sizes="(max-width: 768px) 100vw , 50vw" optionalAlt="Img Project"/>
                                </Link>
                            </div>
                        )}
                            ...
                            <div className="flex flex-wrap -mx-4 gap-y-8">
                                ...
                                {section.linkText && (
                                    <div className="w-full md:w-3/12 2xl:w-4/12 md:pt-2 px-4 flex md:justify-end items-start">
                                        <Link className='flex gap-4 items-center ' href={`./projects/${project.project?.slug.current}`}>
